...
public class MainActivityStable extends Activity {
    ...
    private void base(){
        ...
        root.setPadding(dp(28),dp(14),dp(28),dp(18));
        ...
        TextView logo = txt("▶ CineDeck RU 1.1.6 Stable",24,Color.WHITE,true);
    ...
    private void showHome(){
        base();
        root.addView(txt("Каталог из интернета",30,Color.WHITE,true));
        root.addView(txt("JSON и обложки загружаются через GitHub CDN/Raw. В APK контента нет.",15,MUTED,false));
        root.addView(space(12));
        ProgressBar p = new ProgressBar(this);
        root.addView(p,new LinearLayout.LayoutParams(dp(44),dp(44)));
        TextView status = txt("Загружаю каталог…",16,MUTED,false);
        root.addView(status);
        ...
        io.execute(() -> {
            ...
            main.post(() -> {
                ...
                if(items.isEmpty()){
                    ...
                    content.addView(txt("Открой «Диагностика»: она покажет отдельно CDN JSON, Raw JSON, версию feed и тест обложки.",15,MUTED,false));
                    if(!lastError.isEmpty()) content.addView(txt("Ошибка: "+lastError,13,MUTED,false));
                    return;
                }
                content.addView(txt("Получено: "+items.size()+" позиций • источник: "+selectedSource,14,MUTED,false));
                List<Item> movies=new ArrayList<>(), series=new ArrayList<>();
                for(Item i:items){
                    if("movie".equals(i.type) && movies.size()<24) movies.add(i);
                    if(!"movie".equals(i.type) && series.size()<24) series.add(i);
                }
                if(!movies.isEmpty()){
    ...
    private void showDiag(){
        ...
        TextView out=txt("Проверяю…",17,MUTED,false);
        root.addView(out);
        io.execute(() -> {
            ...
            if(!items.isEmpty()){
                ...
                Bitmap b=tryBitmap(first.poster);
                String route="CDN";
                if(b==null){ b=tryBitmap(first.posterRaw); route="RAW"; }
                image=b==null?"ОШИБКА":"OK "+b.getWidth()+"×"+b.getHeight()+" ("+route+")";
            }
            final String res =
                    "GitHub CDN JSON: "+cdn.status+"\n"+
                    "Версия CDN feed: "+cdn.version+"\n"+
                    "GitHub Raw JSON: "+raw.status+"\n"+
                    "Версия Raw feed: "+raw.version+"\n"+
                    "Каталог распознан: "+items.size()+"\n"+
                    "Источник каталога: "+(selectedSource.isEmpty()?"—":selectedSource)+"\n"+
                    "Тест обложки: "+image+
    ...
    private List<Item> loadFeedFresh(){
        for(String u:new String[]{FEED,FEED_RAW}){
            try{
                ...
                if(version<3){
                    lastError="Получен устаревший feed v"+version;
                    continue;
                }
    ...
    private View card(Item i){
        ...
        im.setBackgroundColor(Color.rgb(28,36,48));
    ...
    private void showDetails(Item i){
        ...
        info.setPadding(dp(20),0,0,0);
        info.addView(txt(i.name,30,Color.WHITE,true));
        info.addView(txt(i.year+(i.rating.isEmpty()?"":" • ★ "+i.rating),15,MUTED,false));
    ...
    private Bitmap tryBitmap(String u){
        ...
        try{
            ...
            c.setRequestProperty("User-Agent","Mozilla/5.0 (Android TV) CineDeck/1.1.6");
            c.connect();
    ...
    static class Probe{ String status=""; int version=0; }
}
