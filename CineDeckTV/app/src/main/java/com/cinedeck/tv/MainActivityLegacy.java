...
public class MainActivityLegacy extends Activity {
    ...
    private static final String PREFS = "cinedeck_legacy";
    private static final String KEY_M3U = "m3u";
    private static final String API_HTTPS = "https://api.tvmaze.com";
    private static final String API_HTTP = "http://api.tvmaze.com";

    private final ExecutorService io = Executors.newFixedThreadPool(3);
    private final Handler main = new Handler(Looper.getMainLooper());
    private final LinkedHashMap<String, Bitmap> imageCache = new LinkedHashMap<String, Bitmap>(16, .75f, true) {
        @Override protected boolean removeEldestEntry(Map.Entry<String, Bitmap> eldest) { return size() > 12; }
    };
    ...
    private void topBar() {
        ...
        TextView logo = text("▶  CineDeck RU Legacy", 24, Color.WHITE, true);
        bar.addView(logo, new LinearLayout.LayoutParams(dp(320), dp(58)));
    ...
    private void showHome() {
        ...
        hero.addView(text("CINEDECK RU 1.1.1 LEGACY", 13, PRIMARY, true));
        hero.addView(text("Версия для старых Android TV / TV Box", 31, Color.WHITE, true));
        hero.addView(text("Облегчённый режим: меньше памяти, системный видеоплеер и резервный HTTP-доступ к каталогу.", 16, Color.LTGRAY, false));
        LinearLayout a = row();
        a.addView(action("⌕  Поиск", v -> showSearch()));
        a.addView(action("▣  ТВ / M3U", v -> showChannels()));
        a.addView(action("◉  Проверка устройства", v -> showDiagnostics()));
        hero.addView(a);
        root.addView(hero, new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(190)));
        root.addView(spacer(12));
        addServices(root, null);
        root.addView(spacer(12));

        ScrollView sc = new ScrollView(this);
        LinearLayout content = column();
        sc.addView(content);
        content.addView(text("Популярные сериалы", 24, Color.WHITE, true));
        ...
        io.execute(() -> {
            List<Show> shows = loadPopular();
            main.post(() -> {
                content.removeView(p);
                if (shows.isEmpty()) {
                    holder.addView(text("Каталог не загрузился. Открой «Диагностика» — там будет видно, что именно не отвечает.", 16, MUTED, false));
                    holder.addView(action("Открыть диагностику", v -> showDiagnostics()));
                } else holder.addView(showRail(shows));
            });
        });
    }
    ...
    private void showSearch() {
        ...
        root.addView(text("Поиск идёт через облегчённый каталог TVMaze; ниже можно сразу искать тот же запрос в российских сервисах.", 14, MUTED, false));
        ...
        Runnable search = () -> {
            ...
            io.execute(() -> {
                ...
                main.post(() -> {
                    ...
                    if (found.isEmpty()) results.addView(text("Каталог ничего не вернул. Российские сервисы выше всё равно можно открыть.", 16, MUTED, false));
    ...
    private View card(Show s) {
        ...
        loadImage(image, s.image);
        TextView name = text(s.name, 14, Color.WHITE, true);
        name.setMaxLines(1);
    ...
    private View searchRow(Show s) {
        ...
        r.addView(image, new LinearLayout.LayoutParams(dp(70), dp(96))); loadImage(image, s.image);
        ...
        TextView desc = text(s.summary, 13, Color.LTGRAY, false); desc.setMaxLines(2); info.addView(desc);
    ...
    private void showDetails(Show s) {
        ...
        head.addView(img, new LinearLayout.LayoutParams(dp(205),dp(286))); loadImage(img,s.image);
    ...
    private void showChannels() {
        screen();
        root.addView(text("ТВ / M3U",32,Color.WHITE,true));
    ...
    private void promptM3u() {
        EditText e = new EditText(this); e.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_VARIATION_URI); e.setText(prefs.getString(KEY_M3U,"")); e.setHint("http://.../playlist.m3u");
        new AlertDialog.Builder(this).setTitle("M3U-плейлист").setView(e).setPositiveButton("Сохранить",(d,w)->{prefs.edit().putString(KEY_M3U,e.getText().toString().trim()).apply();showChannels();}).setNegativeButton("Отмена",null).show();
    }

    private void promptStream() {
        EditText e = new EditText(this); e.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_VARIATION_URI); e.setHint("http://.../video.m3u8 или .mp4");
        new AlertDialog.Builder(this).setTitle("Открыть поток").setView(e).setPositiveButton("Открыть",(d,w)->{String u=e.getText().toString().trim();if(!u.isEmpty())play("Поток",u);}).setNegativeButton("Отмена",null).show();
    }

    private void play(String title,String url) {
        screen(); backAction=this::showChannels; root.addView(text(title,22,Color.WHITE,true));
        videoView=new VideoView(this); videoView.setBackgroundColor(Color.BLACK); root.addView(videoView,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,0,1));
        try { videoView.setVideoURI(Uri.parse(url)); android.widget.MediaController mc=new android.widget.MediaController(this); mc.setAnchorView(videoView); videoView.setMediaController(mc); videoView.setOnErrorListener((mp,what,extra)->{Toast.makeText(this,"Поток не поддерживается этой прошивкой",Toast.LENGTH_LONG).show();return true;}); videoView.start(); } catch(Exception e){Toast.makeText(this,"Не удалось открыть поток",Toast.LENGTH_LONG).show();}
    }

    private void stopVideo(){ if(videoView!=null){ try{videoView.stopPlayback();}catch(Exception ignored){} videoView=null; } }

    private void showDiagnostics() {
        screen(); root.addView(text("Диагностика",32,Color.WHITE,true));
        ActivityManager am=(ActivityManager)getSystemService(ACTIVITY_SERVICE);
        int mem=am==null?0:am.getMemoryClass();
        String kernel=System.getProperty("os.version","неизвестно");
        root.addView(text("Android: "+Build.VERSION.RELEASE+" (SDK "+Build.VERSION.SDK_INT+")",17,Color.WHITE,true));
        root.addView(text("Ядро: "+kernel,16,Color.LTGRAY,false));
        ...
        root.addView(spacer(10));
        TextView net=text("Проверяю каталог…",17,MUTED,false); root.addView(net);
        ProgressBar p=new ProgressBar(this); root.addView(p,new LinearLayout.LayoutParams(dp(42),dp(42)));
        io.execute(() -> { boolean https=test(API_HTTPS+"/shows?page=0"); boolean httpOk=test(API_HTTP+"/shows?page=0"); String err=lastNetworkError; main.post(() -> { root.removeView(p); net.setText("HTTPS каталог: "+(https?"OK":"ОШИБКА")+"\nHTTP резерв: "+(httpOk?"OK":"ОШИБКА")+(err.isEmpty()?"":"\nПоследняя ошибка: "+err)); net.setTextColor((https||httpOk)?Color.WHITE:Color.rgb(255,120,120)); }); });
    }

    private boolean test(String u){ try{String s=httpOnce(u,5000,5000);return s!=null&&s.length()>10;}catch(Exception e){lastNetworkError=e.getClass().getSimpleName()+": "+String.valueOf(e.getMessage());return false;} }

    private List<Show> loadPopular() {
        try {
            JSONArray arr = new JSONArray(httpWithFallback("/shows?page=0"));
            List<Show> out = new ArrayList<>();
            for(int i=0;i<arr.length()&&out.size()<18;i++){Show s=parseShow(arr.optJSONObject(i));if(s!=null)out.add(s);} Collections.sort(out,(a,b)->Double.compare(parseRating(b.rating),parseRating(a.rating))); return out;
        } catch(Exception e){ lastNetworkError=e.getClass().getSimpleName()+": "+String.valueOf(e.getMessage()); return Collections.emptyList(); }
    }

    private List<Show> searchShows(String q) {
        try { JSONArray arr=new JSONArray(httpWithFallback("/search/shows?q="+Uri.encode(q))); List<Show> out=new ArrayList<>(); for(int i=0;i<arr.length()&&i<20;i++){JSONObject wrap=arr.optJSONObject(i);Show s=parseShow(wrap==null?null:wrap.optJSONObject("show"));if(s!=null)out.add(s);} return out; } catch(Exception e){lastNetworkError=e.getClass().getSimpleName()+": "+String.valueOf(e.getMessage());return Collections.emptyList();}
    }

    private Show parseShow(JSONObject o){ if(o==null)return null; Show s=new Show(); s.name=o.optString("name",""); if(s.name.isEmpty())return null; s.id=o.optInt("id",0); String premiered=o.optString("premiered",""); s.year=premiered.length()>=4?premiered.substring(0,4):""; JSONObject r=o.optJSONObject("rating"); s.rating=r==null?"":String.valueOf(r.optDouble("average",0)); if("0.0".equals(s.rating)||"NaN".equals(s.rating))s.rating=""; JSONObject imgs=o.optJSONObject("image"); if(imgs!=null)s.image=imgs.optString("medium",imgs.optString("original","")); s.summary=clean(o.optString("summary","Описание отсутствует.")); JSONArray g=o.optJSONArray("genres"); if(g!=null){StringBuilder b=new StringBuilder();for(int i=0;i<g.length()&&i<3;i++){if(b.length()>0)b.append(", ");b.append(g.optString(i));}s.genres=b.toString();} return s; }

    private double parseRating(String s){try{return Double.parseDouble(s);}catch(Exception e){return 0;}}

    private String httpWithFallback(String path) throws Exception {
        try { return httpOnce(API_HTTPS+path,7000,10000); }
        catch(Exception first){ lastNetworkError=first.getClass().getSimpleName()+": "+String.valueOf(first.getMessage()); return httpOnce(API_HTTP+path,7000,10000); }
    }

    private String httpOnce(String address,int connect,int read) throws Exception {
        HttpURLConnection c=(HttpURLConnection)new URL(address).openConnection(); c.setConnectTimeout(connect); c.setReadTimeout(read); c.setInstanceFollowRedirects(true); c.setRequestProperty("User-Agent","Mozilla/5.0 CineDeckLegacy/1.1.1");
        try(InputStream in=c.getInputStream();ByteArrayOutputStream out=new ByteArrayOutputStream()){byte[] buf=new byte[4096];int n;while((n=in.read(buf))!=-1)out.write(buf,0,n);return new String(out.toByteArray(),Charset.forName("UTF-8"));} finally{c.disconnect();}
    }

    private void loadImage(ImageView target,String url){ if(url==null||url.isEmpty())return; synchronized(imageCache){Bitmap c=imageCache.get(url);if(c!=null){target.setImageBitmap(c);return;}} io.execute(() -> { try{HttpURLConnection con=(HttpURLConnection)new URL(url).openConnection();con.setConnectTimeout(6000);con.setReadTimeout(8000);con.setRequestProperty("User-Agent","Mozilla/5.0 CineDeckLegacy/1.1.1");BitmapFactory.Options opt=new BitmapFactory.Options();opt.inPreferredConfig=Bitmap.Config.RGB_565;opt.inSampleSize=2;Bitmap b;try(InputStream in=con.getInputStream()){b=BitmapFactory.decodeStream(in,null,opt);}con.disconnect();if(b!=null){synchronized(imageCache){imageCache.put(url,b);}main.post(()->{if(!isFinishing())target.setImageBitmap(b);});}}catch(Throwable ignored){} }); }

    private List<Channel> loadM3u(String url){ try{String raw=httpOnce(url,8000,12000);String[] lines=raw.replace("\r","").split("\n");List<Channel> out=new ArrayList<>();String name=null;for(String line:lines){line=line.trim();if(line.startsWith("#EXTINF")){int c=line.lastIndexOf(',');name=c>=0?line.substring(c+1).trim():"Канал";}else if(!line.isEmpty()&&!line.startsWith("#")&&(line.startsWith("http://")||line.startsWith("https://"))){out.add(new Channel(name==null?"Канал "+(out.size()+1):name,line));name=null;if(out.size()>=80)break;}}return out;}catch(Exception e){lastNetworkError=e.getClass().getSimpleName()+": "+String.valueOf(e.getMessage());return Collections.emptyList();} }
    ...
    private static class Show{int id;String name="",year="",rating="",image="",summary="",genres="";}
